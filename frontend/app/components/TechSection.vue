<template>
  <section id="tech" class="py-24 px-6 lg:px-[60px] bg-surface-alt">
    <div class="max-w-container mx-auto overflow-hidden">
      ...
      <div class="text-center max-w-[620px] mx-auto mb-14">
        <div class="flex items-center justify-center gap-2.5 mb-3.5">
          ...
          <div class="text-xs font-bold text-purple tracking-[2px] uppercase">Технологии</div>
        </div>
        <h2 class="font-heading font-extrabold text-[clamp(28px,3.5vw,44px)] tracking-[-1.5px] leading-[1.1] text-ink mb-3.5">
          Мы работаем со всеми современными стеками
        </h2>
        <p class="text-base text-ink-3 leading-[1.75]">
          Технологии и инструменты, которые используют наши IT-специалисты в аутстаф-проектах, обеспечивая стабильное финансирование клиники.
        </p>
      </div>
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3.5 mb-8">
        <div
          ...
          :class="[statBg[stat.color], statSpan[stat.color]]"
        >
          <div class="font-heading font-black text-[42px] leading-none tracking-[-2px]" :class="statNumColor[stat.color]">
            {{ stat.value }}
          </div>
          <div class="text-xs font-semibold mt-1" :class="statLabelColor[stat.color]">
            {{ stat.label }}
          </div>
        </div>
      ...
      <div class="relative w-full overflow-hidden flex py-2">
        ...
        <div class="flex gap-4 w-max animate-marquee hover:[animation-play-state:paused]">
          ...
          <div
            ...
            :class="[themeBg[tech.theme], 'hover:shadow-[0_16px_40px_rgba(0,0,0,0.12)]']"
          >
            <div class="w-[52px] h-[52px] rounded-2xl flex items-center justify-center font-heading text-base font-black text-white mb-3.5" :class="themeIconBg[tech.theme]">
              {{ tech.icon }}
            </div>
            <div class="font-heading text-base font-extrabold" :class="themeText[tech.theme]">{{ tech.name }}</div>
            <div class="text-[11px] font-semibold uppercase tracking-wide opacity-70" :class="themeSub[tech.theme]">{{ tech.sub }}</div>
          </div>
...
<script setup>
import { techStats, techStack } from '~/data/tech'

// Statistic mosaic colors
const statBg = {
...
const statSpan = { blue: '', pink: '', green: '', purple: '' }
